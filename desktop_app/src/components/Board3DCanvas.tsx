...
import type { CameraCommand, GameState, HintResult, LayerSpacing, Move, PieceFocus, Player } from "../types";
...
function CameraRig({ command, spacing }: { command: CameraCommand; spacing: LayerSpacing }) {
  ...
  useEffect(() => {
    ...
    if ("zoom" in camera) {
      camera.zoom = command.preset === "top"
        ? 64
        : spacing === "expanded"
  ...
  }, [camera, command, invalidate, spacing]);
...
function BoardPieces({ state, focus, spacing }: { state: GameState; focus: PieceFocus; spacing: LayerSpacing }) {
...
function WinningLine({ state, spacing }: { state: GameState; spacing: LayerSpacing }) {
...
function InteractiveColumns(props: ColumnsProps) {
  ...
  const legal = useMemo(() => legalMovesByColumn(props.moves), [props.moves]);
...
function AxisLabel({ text, position }: { text: string; position: [number, number, number] }) {
  const texture = useMemo(() => {
    ...
    if (context) {
      context.fillStyle = "rgba(5, 19, 32, 0.9)";
      context.fillRect(4, 4, 120, 56);
      context.strokeStyle = "rgba(80, 215, 220, 0.5)";
      context.lineWidth = 3;
      context.strokeRect(5.5, 5.5, 117, 53);
      context.fillStyle = "#9af1f3";
  ...
  return (
    <sprite position={position} scale={[0.62, 0.31, 1]}>
      <spriteMaterial map={texture} transparent depthTest={false} depthWrite={false} />
    </sprite>
  );
}

function CoordinateLabels({ spacing, visible }: { spacing: LayerSpacing; visible: boolean }) {
  ...
  return (
    <group>
      {Array.from({ length: BOARD_SIZE }, (_, col) => {
        ...
        return <AxisLabel key={`c${col}`} text={`C${col + 1}`} position={[x, y, half + 0.28]} />;
      })}
      {Array.from({ length: BOARD_SIZE }, (_, row) => {
        const [, , z] = boardToWorld(0, row, 0, spacing);
        return <AxisLabel key={`r${row}`} text={`R${row + 1}`} position={[-half - 0.28, y, z]} />;
      })}
    </group>
  );
}
...
function Scene(props: Props & { hoveredMove: Move | null; setHoveredMove: (move: Move | null) => void }) {
  ...
  const hintVisible = props.hint && props.state.board[props.hint.move.layer]?.[props.hint.move.row]?.[props.hint.move.col] === 0;
  ...
  return (
    <>
      ...
      <CameraRig command={props.cameraCommand} spacing={props.layerSpacing} />
      <BoardLattice spacing={props.layerSpacing} guidesVisible={props.showColumnGuides} />
      <CoordinateLabels spacing={props.layerSpacing} visible={props.showColumnGuides} />
      <InteractiveColumns
        ...
        onMove={props.onMove}
      />
      <BoardPieces state={props.state} focus={props.pieceFocus} spacing={props.layerSpacing} />
      <WinningLine state={props.state} spacing={props.layerSpacing} />
...
export function Board3DCanvas(props: Props) {
  ...
  }, [props.state.revision]); // eslint-disable-line react-hooks/exhaustive-deps
