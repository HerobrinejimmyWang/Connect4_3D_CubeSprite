...
| --- | --- | --- |
| `Connect4 3D CubeSprite_0.1.0_x64-setup.exe` | Windows desktop installer | To be recorded for the final build |
| `Connect4 3D CubeSprite_0.1.0-beta.apk` | **Android Beta** test build | To be recorded for the final build |
| `Connect4_3D_CubeSprite_models_v0.1.0.zip` | Original PyTorch `.pth` weights, manifest, and checksums | To be recorded after packaging |
...
- [ ] Confirm the four bundled model names, iterations, and SHA-256 hashes.
- [ ] Package the original `.pth` state dictionaries, model manifest, and
      `SHA256SUMS.txt`.
- [ ] Review model provenance and third-party license notices.
- [ ] Run frontend type checking and the complete frontend test suite.
...
- [ ] Build the Android Beta APK and smoke-test it on a supported Android device.
...
- [ ] Record final installer size and SHA-256.
